          ...
          if (cartItems[item._id] > 0) {
            return (
              <div key={item._id} className="">
                <div
                  className="cart-items-title cart-items-item grid grid-cols-6 items-center text-[#ff6347] text-xs mx-4"
                  style={{ margin: "10px 0", color: "black" }}
                >
                  <img src={item.image} alt="" className="w-10" />
                  <p className="text-[#ff6347]">{item.name}</p>
                  <p className="text-[#ff6347] font-bold">${item.price}</p>
                  <p className="text-[#ff6347]">{cartItems[item._id]}</p>
                  <p className="text-[#ff6347] font-bold">
                    ${item.price * cartItems[item._id]}
                  </p>
                  <p
                    className="cross cursor-pointer text-[#ff6347] bg-gray-100 w-[15%] hover:text-[20px] transition-[3s] text-center rounded-xl p-1"
            ...
            <div>
              ...
              <div className="cart-promocode-input mt-5 flex justify-between items-center bg-white rounded-md">
                <input
                  ...
                  className="bg-gray-200 border-none outline-none pl-10 h-11"
                />
                <button className="w-max10vw md:w-150px px-5 py-3 bg-black border-none text-white rounded-md">
                  Submit
                </button>
              </div>
            </div>
